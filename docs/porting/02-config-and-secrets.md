...
| `debug_prints` | bool | `true` | Enables debug logging; same lenient string parsing (grid-launcher.py:2150-2156). **rewrite**: `debug_prints` switches the tracing filter between `info` and `debug` at runtime; `RUST_LOG` wins when set. |
