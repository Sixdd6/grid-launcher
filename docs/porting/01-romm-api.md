...
| 2 | GET | `/api/platforms` | none | Expects a JSON array. Reads `id`, `slug`, `name`/`display_name`, `rom_count`, `url_logo` per entry (`grid_launcher/server/catalog.py:37`, `:67`, `:98`). The rewrite's `grid_core::romm::Platform` (`crates/grid-core/src/romm/mod.rs`) now also reads `custom_name` and `display_name`; `Platform::label()` picks the first non-empty of `display_name`, `name`, `slug`, matching `catalog.py`'s label rule | present; spec also allows `updated_after`, unused |
