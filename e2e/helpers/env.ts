/**
 * Per-stage environment, handed down by `scripts/e2e.sh`.
 *
...
function required(name: string): string {
  ...
  if (!value || value.trim() === '') {
    throw new Error(
      `${name} is not set. Run the E2E suite through scripts/e2e.sh, ` +
 ...
 * `scripts/check_secret_hygiene.sh` allowlists this exact literal.
 */
 ...
 * mock-romm/server.mjs and scripts/e2e.sh's mock_args_for_group).
