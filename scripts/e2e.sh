...
#   E2E_SKIP_BUILD=1 scripts/e2e.sh   # skips the Rust AND frontend
#                                             # builds: safe ONLY when nothing
#                                             # under app/src or the
...
APP_BINARY="$REPO_DIR/target/debug/app"
BUILD_STAMP="$REPO_DIR/target/debug/.e2e-build-stamp"
...
# `scripts/e2e.sh library downloads` — already supported by the
...
if [[ "${E2E_INNER:-}" != "1" ]]; then
  ...
  if [[ "${E2E_SKIP_BUILD:-}" == "1" ]]; then
    ...
    # nothing under app/src or the Rust crates has changed since the
