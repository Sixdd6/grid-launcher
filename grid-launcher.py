...
from grid_launcher.library import (
...
from grid_launcher.library.cloud_transfer import SUPPORTED_IMAGE_EXTENSIONS, session_screenshot_path
from grid_launcher.library.firmware_install import download_ps3_firmware_direct, install_platform_firmware
from grid_launcher.cover import (
...
from grid_launcher.emulator.retroarch import retroarch_core_flags as resolve_retroarch_core_flags
from grid_launcher.emulator.retroarch import retroarch_core_flags_for_platform as resolve_retroarch_core_flags_for_platform
from grid_launcher.emulator.retroarch import flycast_vmu_file_candidates as resolve_flycast_vmu_file_candidates
...
class MainWindow(CloudSaveMixin, EmulatorUIMixin, InstallMixin, DetailsViewMixin, QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        _fw_logger = _logging.getLogger("grid_launcher.library.firmware_install")
        ...
        self.compat_tool_button_group: QButtonGroup | None = None
        ...
        self.server_platform_slug_map: dict[str, str] = {}
    ...
    def _switch_to_tv_mode_widget(self) -> None:
        if self._tv_window is None:
            ...
            from grid_launcher.tv.bridge.cloud_backend import CloudBackend
            ...
            from grid_launcher.tv.widgets.window import TVWindow
    ...
    def _build_emulators_page(self) -> QWidget:
        ...
        if sys.platform != "win32":
            ...
            compat_tool_button_group = QButtonGroup(compat_tool_list)
            compat_tool_button_group.setExclusive(True)

            self.compat_tool_list = compat_tool_list
            self.compat_tool_button_group = compat_tool_button_group
    ...
    def _build_settings_page(self) -> QWidget:
        ...
        self.settings_status_label = QLabel("Loaded settings from ~/.grid-launcher/config.json")
    ...
    def _config_dir(self) -> Path:
        return Path.home() / ".grid-launcher"
    ...
    def _ra_login_clicked(self) -> None:
        ...
        from grid_launcher.background.workers import RALoginWorker
    ...
    def _load_server_games(self, platform_label: str) -> None:
        ...
        import threading
        from grid_launcher.core.api import api_get_json
        from grid_launcher.server.catalog import fetch_platform_rom_items, games_from_rom_items
        from grid_launcher.cover.utils import cover_url_from_rom_payload, screenshot_urls_from_rom_payload, resolve_cover_url
    ...
    def _is_xbox360_platform(self, game: dict[str, str]) -> bool:
        from grid_launcher.emulator import is_xbox360_platform
    ...
    def _emulator_supports_platform(self, emulator: dict[str, str], platform: str) -> bool:
        ...
        if profile is not None and bool(profile.get("all_platforms", False)):
            return True

        if is_retroarch:
            return bool(
                self._installed_retroarch_cores_for_platform(
                    selected_platform, emulator_name, emulator_entry=emulator
                )
            )
...
def main() -> None:
    ...
    _singleton_name = "grid-launcher-singleton"
    _probe = QLocalSocket()
