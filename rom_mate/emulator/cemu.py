...
def ensure_cemu_controller_config(emulator_path_text: str) -> dict:
    """Write default Pro Controller profile to portable/controllerProfiles/controller0.xml if not already present.

    Windows uses the XInput profile; all other platforms use the SDLController profile.
    """
    changed = False
    ...
    try:
        ...
        if sys.platform == "win32":
            profile_content = _DEFAULT_CEMU_XINPUT_CONTROLLER_PROFILE
        else:
            profile_content = _DEFAULT_CEMU_SDL_CONTROLLER_PROFILE
