...
def _extract_zip_with_7z(
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="grid-launcher-save-7z-"))
...
def extract_zip_archive_bytes_to_directory(
    ...
    *,
    ...
    try:
        fd, temp_path = tempfile.mkstemp(prefix="grid-launcher-save-", suffix=".zip")
        os.close(fd)
...
def zip_native_save_dirs_for_upload(
    ...
    The archive always contains ``_grid_launcher_dirs.json`` even when no files
    ...
    try:
        with zipfile.ZipFile(archive_path, mode="w", compression=zipfile.ZIP_DEFLATED) as archive:
            ...
            archive.writestr("_grid_launcher_dirs.json", manifest_bytes)
    except OSError:
