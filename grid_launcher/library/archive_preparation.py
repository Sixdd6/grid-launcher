...
_KNOWN_7Z_PATHS = [
    ...
    "/usr/bin/7za",
    "/usr/bin/7zz",
    "/usr/lib/p7zip/7za",       # some older distros
    "/opt/homebrew/bin/7z",     # macOS Homebrew (Apple Silicon)
    "/usr/local/bin/7z",        # macOS Homebrew (Intel) / manual installs
    "/usr/local/bin/7za",
]
...
def _try_system_7z(archive_path: Path, extracted_dir: Path) -> bool:
    ...
    for known in _KNOWN_7Z_PATHS:
        if known not in seen and Path(known).is_file():
            seen.add(known)
...
def apply_xenia_content_archive_without_ui(
    ...
    *,
    ...
    """
    from grid_launcher.emulator.xenia import apply_xenia_content_without_ui as _apply_stfs
...
def native_extracted_dir_for_archive_path(archive_path: Path) -> Path:
    """Return the fixed extraction directory for a native game archive.

    Always resolves to <parent>/game regardless of the archive filename,
    keeping the layout consistent:  game_home_dir/game/
    """
    return archive_path.parent / "game"
...
def prepare_installed_game_without_ui(
    ...
    *,
    ...
    if sys.platform != "win32" and extracted_file.is_file():
        try:
