    ...
    *,
    ...
    if is_native_executable_platform(game):
        ...
        if isinstance(native_game_dir_value, str) and native_game_dir_value.strip():
            native_game_dir = Path(native_game_dir_value)
            if native_game_dir.exists() and native_game_dir.is_dir():
                try:
                    remove_directory_tree(native_game_dir)
                except OSError as error:
                    raise OSError(f"Could not remove folder: {native_game_dir}\n{error}") from error
                return
        for extracted_dir in candidate_extracted_dirs_for_game(game):
            if not extracted_dir.exists() or not extracted_dir.is_dir():
                continue
