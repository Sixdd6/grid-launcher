...
from grid_launcher.library import extract_archive_into_directory
from grid_launcher.library.firmware_install import download_ps3_firmware_direct, install_platform_firmware
from grid_launcher.ui import (
...
class EmulatorUIMixin:
    ...
    def _default_compat_tool(self) -> str:
        value = self.config.get("default_compat_tool", "")
        return value.strip() if isinstance(value, str) else ""

    def _set_default_compat_tool(self, path: str) -> None:
        self.config["default_compat_tool"] = path
        self._save_config(self.config)

    def _on_compat_tool_default_toggled(self, checked: bool, path: str) -> None:
        if checked:
            self._set_default_compat_tool(path)
    ...
    def _scan_system_proton_installs(self) -> list[dict[str, str]]:
        scan_paths = [
        ...
        for base in scan_paths:
            ...
            for subdir in sorted(base.iterdir()):
                if not subdir.is_dir():
                    continue
                proton_binary = subdir / "proton"
                if not proton_binary.is_file():
                    continue
                ...
                found.append({"name": subdir.name, "type": "proton", "path": real_str})
        return found
    ...
    def _available_compat_tools_for_dialog(self) -> list[dict[str, str]]:
        ...
        for entry in self._scan_system_proton_installs():
            if entry["path"] in managed_paths:
                continue
            tools.append(entry)
        for entry in self._compat_tool_installs().values():
    ...
    def _refresh_compat_tool_list(self) -> None:
        compat_tool_list = getattr(self, "compat_tool_list", None)
        if compat_tool_list is None:
            return

        button_group = getattr(self, "compat_tool_button_group", None)

        # Clear existing rows.
        if button_group is not None:
            for button in list(button_group.buttons()):
                button_group.removeButton(button)
        compat_tool_list.clear()

        default_path = self._default_compat_tool()
        added_any = False

        def _add_compat_tool_radio(tool_path: str, display_text: str) -> None:
            nonlocal added_any
            radio = QRadioButton(display_text)
            radio.setProperty("tool_path", tool_path)
            if button_group is not None:
                button_group.addButton(radio)
            if tool_path == default_path:
                radio.setChecked(True)
            radio.toggled.connect(
                lambda checked, p=tool_path: self._on_compat_tool_default_toggled(checked, p)
            )
            ...
            compat_tool_list.addItem(item)
            compat_tool_list.setItemWidget(item, row_widget)
            item.setSizeHint(row_widget.sizeHint())
            added_any = True

        # System Wine — selectable radio button.
        wine_path = shutil.which("wine")
        if wine_path:
            _add_compat_tool_radio("wine", f"Wine (system) — {wine_path}")

        # System Proton installs — selectable radio buttons.
        managed_paths = {
            entry.get("install_path", "")
            for entry in self._compat_tool_installs().values()
        }
        for entry in self._scan_system_proton_installs():
            if entry["path"] in managed_paths:
                continue
            _add_compat_tool_radio(
                entry["path"], f"{entry['name']} (system) — {entry['path']}"
            )

        # Managed installs — selectable radio buttons.
        for entry in self._compat_tool_installs().values():
            install_path = entry.get("install_path", "")
            if not install_path:
                continue
            _add_compat_tool_radio(install_path, f"{entry['name']} — {install_path}")

        if not added_any:
            placeholder = QListWidgetItem("No compatibility tools installed")
            placeholder.setFlags(Qt.ItemFlag.NoItemFlags)
            compat_tool_list.addItem(placeholder)
    ...
    def _emulator_autoprofiles_path(self) -> Path:
        return emulator_autoprofiles_path(grid_launcher_share_dir(Path(__file__).resolve().parents[3]))
    ...
    def _retroarch_slug_core_map(self) -> dict:
        cache = getattr(self, "_retroarch_slug_core_map_cache", None)
        if isinstance(cache, dict):
            return cache
        path = retroarch_slug_core_map_path(grid_launcher_share_dir(Path(__file__).resolve().parents[3]))
        slug_core_map = load_retroarch_slug_core_map(path)
        self._retroarch_slug_core_map_cache = slug_core_map
        return slug_core_map
    ...
    def _retroarch_installed_core_ids_for_emulator(
        self,
        emulator_name: str,
        emulator_entry: dict | None = None,
    ) -> set[str]:
    ...
    def _installed_retroarch_cores_for_platform(
        self,
        platform: str,
        emulator_name: str,
        emulator_entry: dict | None = None,
        ...
        installed_core_ids = self._retroarch_installed_core_ids_for_emulator(
            emulator_name, emulator_entry=emulator_entry
        )
    ...
    def _retroarch_cores_for_platform(self, platform: str) -> list[str]:
        slug = getattr(self, "server_platform_slug_map", {}).get(platform, "")
        if slug:
            return retroarch_cores_for_slug(slug, self._retroarch_slug_core_map())
        # Fuzzy fallback for offline/local library
    ...
    def _available_source_download_emulator_entries(
        ...
        source_rows = available_source_download_emulator_entries(
            autoprofiles,
            query=query,
    ...
    def _backfill_missing_emulator_defaults(self) -> None:
        ...
        ``assign_profile_platform_defaults`` never overwrites an existing
        default, so calling it here is a safe no-op for emulators that already
        have their defaults set.  This backfills defaults for source-installed
        emulators (e.g. RetroArch) whose cores/data only became available after
        the initial install, once server platform data is loaded.
        """
        original_defaults = self._normalize_default_emulators(
            self.config.get("default_emulators", {})
        )
        original_core_defaults = self._normalize_default_retroarch_cores(
        ...
        final_defaults = self._normalize_default_emulators(
            self.config.get("default_emulators", {})
        )
        final_core_defaults = self._normalize_default_retroarch_cores(
            self.config.get("default_retroarch_cores", {})
        )
        if final_defaults != original_defaults or final_core_defaults != original_core_defaults:
            self._save_config(self.config)
