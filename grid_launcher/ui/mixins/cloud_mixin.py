...
from grid_launcher.background import AutoCloudSaveUploadWorker
...
class CloudSaveMixin:
    ...
    def _resolved_sync_directory_paths(self, emulator: dict[str, str], key: str) -> list[Path]:
        ...
        if not configured_paths and key == "save_paths" and self._emulator_matches_tokens(emulator_name, "vita3k", emulator=emulator):
            vita3k_override_paths = vita3k_save_path_overrides(
                emulator.get("path", ""),
                emulator.get("args", ""),
                self._split_launch_template_args,
            )
            if vita3k_override_paths:
                merged_paths: list[str] = []
                for raw_path in [*vita3k_override_paths, *all_paths]:
                    if not isinstance(raw_path, str) or not raw_path.strip():
                        continue
                    if raw_path not in merged_paths:
                        merged_paths.append(raw_path)
        ...
        try:
            ...
            for record in records:
                ...
                if emulator_field == "native_multi_dir":
                    ...
                    fd, tmp_path = tempfile.mkstemp(prefix="grid-launcher-restore-", suffix=".zip")
                    os.close(fd)
                    ...
                    try:
                        ...
                        with _zipfile.ZipFile(tmp_zip, "r") as archive:
                            # Read manifest
                            try:
                                manifest_bytes = archive.read("_grid_launcher_dirs.json")
                            ...
                            for member in archive.infolist():
                                ...
                                if member_name == "_grid_launcher_dirs.json" or member_name.endswith("/"):
                                    continue
