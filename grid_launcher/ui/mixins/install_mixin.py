...
from grid_launcher.library import (
    ...
    native_extracted_dir_for_archive_path,
...
from grid_launcher.library.firmware_install import install_platform_firmware
...
from grid_launcher.ui import make_download_entry_widget, refresh_downloads_page
...
class InstallMixin:
    ...
    def _extract_archive_for_game(
        ...
        if self._is_native_executable_platform(game):
            resolve_extracted_dir = native_extracted_dir_for_archive_path
        else:
            resolve_extracted_dir = self._extracted_dir_for_archive_path
        return extract_archive_for_game(
            game,
            archive_path,
            extracted_dir_for_archive_path=resolve_extracted_dir,
    ...
    def _apply_native_game_update_without_ui(
        ...
        *,
        ...
        from grid_launcher.library.archive_preparation import prepare_native_game_update_without_ui
    ...
    def _native_update_temp_dir_for_game(self, game: dict[str, str]) -> Path:
        ...
        import tempfile
        safe_name = self._sanitize_path_component(game.get("title", "game"), "game")
        return Path(tempfile.gettempdir()) / f"grid-launcher-{safe_name}-temp"
    ...
    def _game_home_dir_for_game(self, game: dict[str, str]) -> Path | None:
        platform_library_dir = self._platform_library_dir(game)
        if platform_library_dir is None:
            return None
    ...
    def _fetch_windows_game_json(self, game: dict[str, str], game_home_dir: Path) -> None:
        ...
        for entry in payload.get("files", []):
            ...
            if entry_name == "game.json":
                game_json_file_id = entry.get("id", "")
                break
        if not game_json_file_id:
            if self._debug_prints_enabled():
                print("[DEBUG][InstallGameJson] no game.json file entry found")
            return
        rom_id_path = quote(rom_id, safe="")
        download_path = f"/api/roms/{rom_id_path}/content/game.json?file_ids={game_json_file_id}"
        try:
            data = self._api_get_bytes(download_path)
            (game_home_dir / "game.json").write_bytes(data)
            parsed = parse_windows_game_json(data)
            apply_windows_game_json_to_game(game, parsed)
        except Exception as error:
            if self._debug_prints_enabled():
                print(f"[DEBUG][InstallGameJson] url={self._server_base_url()}{download_path}")
                print(f"[DEBUG][InstallGameJson] error={error}")
    ...
    def _download_game_archive(self, game: dict[str, str]) -> Path | None:
        ...
        if self._is_native_executable_platform(game):
            game_home_dir = self._game_home_dir_for_game(game)
            if game_home_dir is None:
                QMessageBox.warning(self, "Install Error", "Set a Library Path in Settings before installing games.")
                return None
            try:
                game_home_dir.mkdir(parents=True, exist_ok=True)
            except OSError as error:
                QMessageBox.warning(self, "Install Error", f"Could not prepare library folder: {error}")
                return None
            game["native_game_dir"] = str(game_home_dir)
        else:
            game_home_dir = None
        rom_id_path = quote(rom_id, safe="")

        if game_home_dir is not None:
            ...
            if archive_entry is not None:
                ...
                game["rom_file_name"] = archive_file_name
                archive_path = game_home_dir / archive_file_name
                download_path = (
                    f"/api/roms/{rom_id_path}/content/{quote(archive_file_name, safe='')}"
                    f"?file_ids={archive_entry['id']}"
                )
            else:
                # Fallback for single-file ROMs whose payload has no files array.
        ...
        try:
            payload_bytes = self._api_get_bytes(download_path)
            archive_path.write_bytes(payload_bytes)
            if game_home_dir is not None:
                self._fetch_windows_game_json(game, game_home_dir)
            return archive_path
