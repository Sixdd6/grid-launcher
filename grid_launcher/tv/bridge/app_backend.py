...
class _RomMetaFetchWorker(QObject):
    ...
    def run(self) -> None:
        ...
        from grid_launcher.server.metadata import details_metadata_from_item
        try:
...
class AppBackend(QObject):
    ...
    def connectToServer(self) -> None:
        from grid_launcher.server.state import credentials_present
    ...
    def toggleFavorite(self, rom_id_str: str) -> None:
        ...
        from grid_launcher.tv.bridge.workers import CollectionsFetchWorker
    ...
    def _api_get(self, path: str, params: dict | None) -> Any:
        from grid_launcher.core.api import api_get_json
        from grid_launcher.server.state import server_base_url
    ...
    def _api_put_multipart_text(self, path: str, fields: dict, params=None):
        from grid_launcher.core.api import api_put_multipart_text_json
        from grid_launcher.server.state import server_base_url
    ...
    def _api_post_multipart_text(self, path: str, fields: dict, params=None):
        from grid_launcher.core.api import api_post_multipart_text_json
        from grid_launcher.server.state import server_base_url
    ...
    def _start_favorites_fetch(self) -> None:
        from grid_launcher.tv.bridge.workers import FavoritesRomFetchWorker
        base_url = self._config.get("server_url", "")
    ...
    def _start_curated_rows_fetch(self) -> None:
        from grid_launcher.tv.bridge.workers import (
    ...
    def _start_saves_fetch(self) -> None:
        from grid_launcher.tv.bridge.workers import SavesBatchFetchWorker
    ...
    def _start_catalog_fetch(self) -> None:
        from grid_launcher.tv.bridge.workers import CatalogFetchWorker
    ...
    def _start_rom_fetch(self, platform_label: str, platform_id: int) -> None:
        from grid_launcher.tv.bridge.workers import RomListFetchWorker
    ...
    def _on_catalog_finished(self, bundle: Any) -> None:
        ...
        from grid_launcher.server.catalog import server_platform_details, server_platform_ids
        from grid_launcher.server.state import account_status_text
    ...
    def _on_catalog_error(self, message: str) -> None:
        from grid_launcher.server.connection import classify_connection_failure
        self._is_connected = False
    ...
    def _on_toggle_collections_fetched(self, result) -> None:
        ...
        if result is None:
            ...
            from grid_launcher.tv.bridge.workers import CollectionCreateWorker
            ...
            if rom_id_int in current_ids:
                ...
                self._toggle_adding = True
            from grid_launcher.tv.bridge.workers import CollectionUpdateWorker
