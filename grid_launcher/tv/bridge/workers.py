...
class RomListFetchWorker(QObject):
    ...
    def run(self) -> None:
        from grid_launcher.cover.utils import cover_url_from_rom_payload, screenshot_urls_from_rom_payload, resolve_cover_url, filter_to_server_host
...
class FavoritesRomFetchWorker(QObject):
    ...
    def run(self) -> None:
        from grid_launcher.cover.utils import cover_url_from_rom_payload, screenshot_urls_from_rom_payload, resolve_cover_url, filter_to_server_host
        from grid_launcher.server.catalog import fetch_rom_items_by_params, games_from_rom_items
        try:
