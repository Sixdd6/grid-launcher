...
def retroarch_slug_core_map_path(base_path: Path) -> Path:
    return base_path / "romm-platform-cores.json"


def load_retroarch_slug_core_map(path: Path) -> dict[str, list[str]]:
    if not path.exists():
        return {}
    try:
    ...
    for slug, cores in data.items():
        if not isinstance(slug, str) or not slug.strip():
            continue
        if not isinstance(cores, list):
            continue
        valid = [c for c in cores if isinstance(c, str) and c.strip()]
        result[slug.strip()] = valid
    return result


def retroarch_cores_for_slug(slug: str, slug_core_map: dict[str, list[str]]) -> list[str]:
    if not slug or not slug_core_map:
        return []
    return list(slug_core_map.get(slug.strip(), []))
...
def retroarch_system_keys_for_platform(platform: str, compatibility: dict[str, list[str]]) -> list[str]:
    ...
    input_tokens = retroarch_platform_tokens(platform)
    if not input_tokens:
        return []
    ...
    for key in compatibility:
        key_tokens = retroarch_platform_tokens(key)
        if not key_tokens:
            continue
        union = input_tokens | key_tokens
        if not union:
            continue
        score = len(input_tokens & key_tokens) / len(union)
        if score > best_score:
            best_score = score
            best_key = key
...
def installed_retroarch_core_ids(
    emulator_path_text: str,
    *,
    cores_dir: "Path | str | None" = None,
    _platform: str | None = None,
) -> set[str]:
    emulator_path = Path(emulator_path_text).expanduser()

    if cores_dir is not None:
        resolved_cores_dir = Path(cores_dir).expanduser()
    else:
        if not emulator_path_text:
            return set()
        if not emulator_path.exists() or not emulator_path.is_file():
            return set()
        appimage_home_cores = (
            emulator_path.parent
            / (emulator_path.name + ".home")
            / ".config"
            / "retroarch"
            / "cores"
        )
        if appimage_home_cores.exists() and appimage_home_cores.is_dir():
            resolved_cores_dir = appimage_home_cores
        else:
            resolved_cores_dir = emulator_path.parent / "cores"

    if not resolved_cores_dir.exists() or not resolved_cores_dir.is_dir():
        return set()
    ...
    for candidate in resolved_cores_dir.glob(glob_pattern):
